...
- 언어: C++20 (`stdcpp20`)
- 빌드: Visual Studio 2026 (MSBuild, PlatformToolset v145) — 주 빌드 시스템
...
| 빌드 | 동작 | 산출물 경로 |
|------|------|------------|
| **Debug** | `InitGoogleMock()` + `RUN_ALL_TESTS()` — 테스트 실행 | `SampleOrderSystem\x64\Debug\SampleOrderSystem.exe` |
| **Release** | `MainController::run()` — 콘솔 앱 실행 | `SampleOrderSystem\x64\Release\SampleOrderSystem.exe` |

### 단일 테스트 실행

```powershell
# 빌드 후 특정 테스트만 실행 (--gtest_filter 사용)
.\SampleOrderSystem\x64\Debug\SampleOrderSystem.exe --gtest_filter="IdGeneratorTest.*"
.\SampleOrderSystem\x64\Debug\SampleOrderSystem.exe --gtest_filter="JsonFileManagerTest.LoadReturnsEmptyArrayWhenFileNotFound"
```
...
| Phase | 내용 | 상태 |
|-------|------|------|
| Phase 1 | 프로젝트 기반 구성 (모델, 유틸, MainController 골격) | ✅ 완료 |
| Phase 2 | 시료 관리 (SampleRepository, SampleService, SampleView, SampleController) | ⏳ 미구현 |
| Phase 3 | 주문 접수 (OrderRepository, OrderService, OrderView, OrderController) | ⏳ 미구현 |
| Phase 4 | 주문 승인/거절 (ProductionJobRepository, ProductionService, ApprovalController) | ⏳ 미구현 |
| Phase 5 | 생산라인 & 출고 (ProductionController, ReleaseService, ReleaseController) | ⏳ 미구현 |
| Phase 6 | 모니터링 & UI 완성 (MonitoringService, MonitoringController, 페이지네이션) | ⏳ 미구현 |
| Phase 7 | 더미 데이터 & 최종 품질 리팩토링 | ⏳ 미구현 |
...
### Model (`src/model/`)
- `Sample` — `{sampleId, name, avgProdTime, yield, stock}`, `NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE` 매크로로 JSON 직렬화
- `Order` — `{orderId, sampleId, customerName, quantity, status, createdAt, updatedAt}`, enum 필드 때문에 수동 `to_json`/`from_json`
- `ProductionJob` — `{jobId, orderId, sampleId, shortage, actualProduction, totalTime, status, estimatedEndTime}`, 수동 직렬화
- `Enums.h` — `OrderStatus`, `JobStatus` enum + `*ToString`/`*FromString` 변환 함수

### Util (`src/util/`)
- `JsonFileManager::load(path)` / `save(path, json)` — 파일 미존재·파싱 오류 시 빈 배열 반환, 부모 디렉토리 자동 생성
- `IdGenerator::nextOrderId()` / `nextSampleId()` / `reset()` — 정적 시퀀스 카운터
- `ConsoleHelper::readInt(min, max)` / `readLine()` / `readDouble()` — 유효성 검증 루프 포함

### Controller (`src/controller/`)
- `MainController::run()` — 메뉴 루프 골격 (각 메뉴 "미구현" 표시)
...
### 테스트 파일 추가 시
vcxproj `<ItemGroup>` 안에 다음 형식으로 등록:
```xml
<ClCompile Include="$(ProjectDir)..\tests\FooTest.cpp">
...
### 소스 파일 추가 시 (src/ 하위)
```xml
<ClCompile Include="$(ProjectDir)..\src\service\FooService.cpp" />
```
...
```cpp
// 단순 구조체 — 매크로 사용 (Sample.h)
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(Sample, sampleId, name, avgProdTime, yield, stock)

// enum 필드 포함 구조체 — 수동 to_json/from_json (Order.h, ProductionJob.h)
inline void to_json(nlohmann::json& j, const Order& o) { /* orderStatusToString 호출 */ }
inline void from_json(const nlohmann::json& j, Order& o) { /* orderStatusFromString 호출 */ }
```

새 모델에 enum 필드가 있으면 반드시 수동 직렬화를 사용한다.
