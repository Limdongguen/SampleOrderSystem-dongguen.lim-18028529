...
### OrderRepository
- [x] `src/repository/OrderRepository.h/.cpp`
...
- [x] `src/service/OrderService.h/.cpp`
...
- [x] `src/view/OrderView.h/.cpp`
  - 주문 입력 프롬프트 (시료 ID, 고객명, 수량)
  - 입력 확인 화면 (시료명 조회 후 표시)
  - 완료 메시지 (주문번호, 현재 상태 RESERVED)
- [x] `src/controller/OrderController.h/.cpp` — 입력 수집 → `OrderService::reserve()` → View 출력
- [x] `MainController`에서 `[2]` 선택 시 `OrderController::reserve()` 연결

### 단위 테스트
- [x] `tests/OrderServiceTest.cpp`
